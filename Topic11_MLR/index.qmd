...
[Luckily, we have a dataset that contains the sale prices, along with other information, for 141 Ebay sales of the Mario Kart Wii game. This data only includes games that sold for less than $100.]{.fragment}
...
Using the statistical tools presented in Topics 9 and 10, we could try to model the `sale` variable using one of the quantitative variables available in the dataset.
...
[For example, $$\hat{\text{sale}} = 42.8167 + 0.1850\text{ start price}$$ is the fitted model when we use only the start price variable to model the final sale price.]{.fragment}
...
[$$\begin{aligned}\hat{\text{sale}} &= 36.89 - 0.50 \text{duration } + 0.19 \text{bids} -4.31 \text {used}\\ &+ 0.16 \text{start price} + 0.00002 \text{seller rate}+6.80 \text{wheels}\end{aligned}$$]{.fragment}

## MLR - Determining the Coefficient Estimates {.smaller}

$$\begin{aligned}\hat{\text{sale}} &= 36.89 -0.50 \text{duration } + 0.19 \text{bids} -4.31 \text {used}\\ &+ 0.16 \text{start price} + 0.00002 \text{seller rate}+6.80 \text{wheels}\end{aligned}$$
...
[$\begin{aligned}\hat{\text{sale}} &= (36.89-4.31) -0.50 \text{duration } + 0.19 \text{bids}\\ &+ 0.16 \text{start price} + 0.00002 \text{seller rate}+6.80 \text{wheels}\end{aligned}$]{.fragment}

-   What does the model look like when considering a "new" game?

[$\begin{aligned}\hat{\text{sale}} &= 36.89 -0.50 \text{duration } + 0.19 \text{bids}\\ &+ 0.16 \text{start price} + 0.00002 \text{seller rate}+6.80 \text{wheels}\end{aligned}$]{.fragment}
...
[When all other explanatory variable values are held constant, we expect final sale price to decrease by \$0.50 for each additional day the game is up for auction.]{.fragment}
...
::: fragment
**Model 1 (6 explanatory variables):** 

Uses duration, bids, condition (used), start price, seller rate, and wheels as explanatory variables
...
::: fragment
**Model 2 (5 explanatory variables):** 

Uses duration, bids, condition (used), start price, and wheels as explanatory variables

$\begin{aligned}\hat{\text{sale}} &= 37.54 -0.58 \text{duration} +0.20 \text{bids} -4.56\text {used}\\ &+ 0.18 \text{start price} +6.46 \text{wheels}\end{aligned}$
:::

```{r}
#| output: false
mlr_small <- lm(sale ~ duration + n_bids + 
                  cond + start_pr + wheels, data = mk)
summary(mlr_small)
```
 
...
$$\hat{\text{interest rate}} = 11.0465 + 0.0494 \text{debt\_to\_income} + 0.3207\text{own} + 0.9819\text{rent}$$
:::
